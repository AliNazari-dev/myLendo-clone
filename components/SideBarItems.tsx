...
const SideBarItems: React.FC<SidebarItemProps> = ({ src, label, active, href, className }) => {
  return (
    <Link
      href={href}
      className={twMerge(
        `flex flex-row h-auto items-center w-full gap-x-4 text-md font-medium cursor-pointer hover:text-blue-700 transition-colors text-neutral-800 py-1`,
      ...
      <p className={`truncate w-full ${className}`}>{label}</p>
    </Link>
  );
};
